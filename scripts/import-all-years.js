/**
 * 全年度（FY2015〜FY2025）のStorageファイルをインポート
 * company_idのNULL問題を解決
 */
...
function extractMetadata(file) {
  ...
  // パターン3: by_company/企業ID_日付_ハッシュ の形式
  ...
  // company_idが見つからない場合は"UNKNOWN_年度_連番"を生成
  if (!companyId) {
 ...
 * 全ファイルを再帰的に取得（改良版）
 */
async function getAllStorageFiles(prefix = '', allFiles = [], depth = 0) {
  // 深さ制限（無限ループ防止）
  if (depth > 10) {
  ...
  try {
    ...
    if (files && files.length > 0) {
      for (const file of files) {
        ...
        // ディレクトリの場合（FY2015〜FY2025を対象）
        else if (!file.name.includes('.')) {
          // 年度フォルダまたはサブフォルダの場合のみ再帰
          if (fullPath.match(/FY20(1[5-9]|2[0-5])/) || depth < 3) {
            await getAllStorageFiles(fullPath, allFiles, depth + 1);
          }
        }
      }
    }
...
async function main() {
  console.log('========================================');
  console.log('📁 全年度Storage Markdownファイルインポート');
  console.log('========================================');
  console.log(`📡 接続先: ${SUPABASE_URL}`);
  console.log(`⚙️ バッチサイズ: ${BATCH_SIZE}`);
  console.log(`📅 対象年度: FY2015〜FY2025`);
  console.log('');
  ...
  console.log(`✅ ${existingPathSet.size}件の既存パスを取得`);
  console.log('');
  ...
  // 新規ファイルのみフィルタ
  const newFiles = allFiles.filter(file => !existingPathSet.has(file.fullPath));
