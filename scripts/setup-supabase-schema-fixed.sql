...
-- フルテキスト検索用インデックス（英語設定を使用）
-- 注: Supabaseは日本語設定をサポートしていないため、simpleまたはenglishを使用
CREATE INDEX IF NOT EXISTS idx_financial_reports_content_search 
  ON financial_reports USING gin(to_tsvector('simple', markdown_content));
...
DROP POLICY IF EXISTS "Financial reports are viewable by everyone" ON financial_reports;
...
DROP POLICY IF EXISTS "Users can only see their own usage logs" ON api_usage_logs;
