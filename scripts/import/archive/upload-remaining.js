...
async function uploadRemainingCompanies() {
  ...
  try {
    ...
    console.log(`✅ 既存: ${existingIds.size}社`);
    
    // 未アップロードの企業を特定
    const remainingCompanies = [];
    for (const companyDir of allCompanies) {
      ...
      if (!existingIds.has(companyId)) {
        remainingCompanies.push(companyDir);
      }
    }
    
    console.log(`📤 アップロード対象: ${remainingCompanies.length}社`);
    console.log('');
    ...
    // バッチ処理開始
    for (let i = 0; i < remainingCompanies.length; i += BATCH_SIZE) {
      const batch = remainingCompanies.slice(i, Math.min(i + BATCH_SIZE, remainingCompanies.length));
      
      console.log(`バッチ ${Math.floor(i / BATCH_SIZE) + 1}: ${i + 1}〜${Math.min(i + BATCH_SIZE, remainingCompanies.length)}社目を処理中...`);
      ...
      // 進捗表示
      console.log(`✅ 進捗: ${processedCount}/${remainingCompanies.length}社完了, エラー: ${errorCount}件`);
      console.log('');
    }
    ...
    console.log('最終統計');
    console.log('===========================================');
    console.log(`📊 総企業数: ${finalCount}社 / ${totalCompanies}社`);
    console.log(`📊 カバー率: ${(finalCount / totalCompanies * 100).toFixed(1)}%`);
  ...
  const { data: reportData, error: reportError } = await supabase
    ...
    .upsert({
      ...
      metadata: { 
        ...
        file_count: mdFiles.length  // ファイル数も記録
      }
    }, {
...
// エラーハンドリング付きで実行
uploadRemainingCompanies().catch(error => {
