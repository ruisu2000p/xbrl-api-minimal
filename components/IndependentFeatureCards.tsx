          ...
          {features.map((feature, index) => (
            <div key={index} className="bg-white rounded-3xl shadow-2xl overflow-hidden">
              <div className={`grid ${index % 2 === 0 ? 'md:grid-cols-2' : 'md:grid-cols-2'}`}>
                ...
                <div className={`relative bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-8 ${index % 2 === 1 ? 'md:order-1' : ''}`}>
                  {activeVideo === index && feature.hasVideo ? (
                      ...
                      {feature.hasVideo && (
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                          <button
                            ...
                            className="bg-white/90 backdrop-blur-sm p-4 rounded-full shadow-xl hover:scale-110 transition-transform"
                          >
                            <i className="ri-play-fill text-4xl text-blue-600"></i>
                          </button>
                        </div>
                      )}
                    </div>
