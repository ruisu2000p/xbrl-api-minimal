...
function ToastContainer({ messages, removeMessage }: ToastProps) {
  return (
    <div className="fixed top-4 right-4 z-50 space-y-2">
      {messages.map((toast) => (
        <Toast key={toast.id} toast={toast} onClose={() => removeMessage(toast.id)} />
      ))}
    </div>
  )
}
...
function Toast({ toast, onClose }: { toast: ToastMessage; onClose: () => void }) {
  ...
  return (
    <div className={`${colors[toast.type]} border rounded-lg shadow-lg p-4 pr-12 max-w-sm animate-slide-in`}>
