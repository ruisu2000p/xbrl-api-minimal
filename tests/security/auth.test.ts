...
describe('Security: Authentication & Authorization', () => {
  ...
  describe('Token Security', () => {
    ...
    it('should prevent token replay attacks', () => {
      ...
      const request1 = new MockNextRequest('http://localhost:3000/api/v1/companies', {
        headers: {
      ...
      const request2 = new MockNextRequest('http://localhost:3000/api/v1/companies', {
        headers: {
          'Authorization': `Bearer ${replayToken}`,
          'X-Timestamp': timestamp.toString()
        }
      ...
      const headers1 = new Headers(request1.headers)
      const headers2 = new Headers(request2.headers)
      expect(headers1.get('X-Timestamp')).toBe(headers2.get('X-Timestamp'))
    })
    ...
    it('should validate token signatures', () => {
      ...
      const tamperedHeaders = new Headers(request.headers)
      expect(tamperedHeaders.get('Authorization')).toContain(tamperedToken)
    })
  })
  ...
  describe('Session Management', () => {
    ...
    it('should rotate session tokens periodically', () => {
      ...
      const rotationHeaders = new Headers(request.headers)
      expect(rotationHeaders.get('Cookie')).toContain(oldToken)
    })
    ...
    it('should enforce session timeout', () => {
      ...
      const activityHeaders = new Headers(request.headers)
      expect(activityHeaders.get('X-Last-Activity')).toBe(lastActivity.toString())
    })
  })
  ...
  describe('Authorization Checks', () => {
    it('should enforce role-based access control', () => {
      ...
      userRoles.forEach(({ role, canDelete }) => {
        const request = new MockNextRequest('http://localhost:3000/api/v1/companies/123', {
          method: 'DELETE',
          headers: {
            'X-User-Role': role
          }
        ...
        const roleHeaders = new Headers(request.headers)
        expect(roleHeaders.get('X-User-Role')).toBe(role)
    ...
    it('should validate resource ownership', () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/v1/api-keys/key_789', {
        method: 'DELETE',
        headers: {
          'X-User-Id': userId,
      ...
      const authzHeaders = new Headers(request.headers)
      expect(authzHeaders.get('X-User-Id')).not.toBe(authzHeaders.get('X-Resource-Owner'))
    })
    ...
    it('should enforce tier-based limitations', () => {
      ...
      tiers.forEach(({ tier, maxRequests }) => {
        ...
        const tierHeaders = new Headers(request.headers)
        expect(tierHeaders.get('X-User-Tier')).toBe(tier)
