...
### 方法1: npx経由でインストール（推奨・最も簡単）

Node.jsがインストールされていれば、ローカルにファイルを置かずに実行できます。
...
タスクマネージャー/アクティビティモニタでClaude.exeが完全に終了していることを確認してから再起動してください。
...
Claude Desktop設定（パスは実際のクローン先に合わせてください）：
...
```json
{
  "mcpServers": {
    "xbrl-local": {
      "command": "node",
      "args": ["./path/to/xbrl-api-minimal/mcp-server/index-unified.js"],
      "env": {
        "XBRL_API_KEY": "your-api-key-here"
      }
    }
...
**注意**: `./path/to/`の部分は実際にクローンしたディレクトリのパスに置き換えてください。
...
#### 設定ファイルの確認
```bash
# Windows: 設定ファイルの構文チェック
type %APPDATA%\Claude\claude_desktop_config.json

# Mac/Linux: 設定ファイルの構文チェック
