...
export async function POST(request: NextRequest) {
  try {
    ...
    const { generateBcryptApiKey } = await import('@/lib/security/bcrypt-apikey')
    const { apiKey, hash, prefix: keyPrefix, suffix: keySuffix, masked } = await generateBcryptApiKey()
