-- Fix crypt function type casting issue
-- Migration: 20251012_fix_crypt_function_cast
...
BEGIN
    ...
    FOR v_key_record IN
        SELECT id, user_id, name, key_hash::text, tier, is_active
        FROM private.api_keys
        WHERE is_active = true
        AND status = 'active'
    LOOP
        -- bcryptでハッシュ比較（明示的な型キャスト）
...
COMMENT ON FUNCTION public.verify_api_key_complete_v2 IS 'APIキーをbcryptハッシュで検証する関数（private.api_keysテーブルを使用、型キャスト修正版）';
