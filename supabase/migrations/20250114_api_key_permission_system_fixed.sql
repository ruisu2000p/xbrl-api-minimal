-- API Key Permission System Migration (Fixed)
...
-- ============================================
-- 2. Update RLS Policies (Drop existing first)
-- ============================================
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Unified read access for companies" ON public.companies;
    DROP POLICY IF EXISTS "Unified read access for markdown metadata" ON public.markdown_files_metadata;
END $$;

-- Companies table: Create new unified policy
CREATE POLICY "API and Auth unified read access for companies"
ON public.companies
FOR SELECT
USING (
    ...
    auth.role() = 'anon'
    OR
    -- Always allow (since this is public data)
    true
);

-- Markdown files metadata: Same unified access
CREATE POLICY "API and Auth unified read access for markdown"
...
BEGIN
    ...
    -- Check for API key in headers (safely)
    BEGIN
        api_key := current_setting('request.headers', true)::jsonb->>'x-api-key';
    EXCEPTION
        WHEN OTHERS THEN
...
-- Index for performance (check if exists first)
...
CREATE INDEX IF NOT EXISTS idx_api_usage_logs_accessed_at ON public.api_usage_logs(accessed_at DESC);
